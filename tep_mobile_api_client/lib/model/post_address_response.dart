...
class PostAddressResponse {
  ...
  @override
  String toString() =>
      'PostAddressResponse[createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, id=$id, parentId=$parentId, name=$name, zipCode=$zipCode, street=$street, neighborhood=$neighborhood, number=$number, state=$state, city=$city, complement=$complement, latitude=$latitude, longitude=$longitude, ie=$ie, type=$type, externalCode=$externalCode, status=$status]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "PostAddressResponse[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "PostAddressResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PostAddressResponse> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<PostAddressResponse>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PostAddressResponse.listFromJson(
          entry.value,
          growable: growable,
        );
      }
