...
class PurchaseOrderItem {
  /// Returns a new [PurchaseOrderItem] instance.
  PurchaseOrderItem({
    this.product,
    this.quantity,
    this.weightTotal,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PurchaseOrderItem &&
  ...
  @override
  String toString() => 'PurchaseOrderItem[product=$product, quantity=$quantity, weightTotal=$weightTotal, unitPrice=$unitPrice, unitPriceQtd=$unitPriceQtd, unitTotalPrice=$unitTotalPrice, percDiscountCommission=$percDiscountCommission, valueDiscount=$valueDiscount, percDiscount=$percDiscount, useValueDiscount=$useValueDiscount, discountType=$discountType, discountByWeight=$discountByWeight, discountTotal=$discountTotal]';
  ...
  /// Returns a new [PurchaseOrderItem] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PurchaseOrderItem? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PurchaseOrderItem[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PurchaseOrderItem[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PurchaseOrderItem> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PurchaseOrderItem>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PurchaseOrderItem.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PurchaseOrderItem> mapFromJson(dynamic json) {
    final map = <String, PurchaseOrderItem>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PurchaseOrderItem.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PurchaseOrderItem-objects as value to a dart map
  static Map<String, List<PurchaseOrderItem>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PurchaseOrderItem>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PurchaseOrderItem.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
