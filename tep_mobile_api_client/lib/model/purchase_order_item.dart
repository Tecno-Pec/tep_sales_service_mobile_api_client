...
class PurchaseOrderItem {
  ...
  @override
  String toString() =>
      'PurchaseOrderItem[product=$product, quantity=$quantity, weightTotal=$weightTotal, unitPrice=$unitPrice, unitPriceQtd=$unitPriceQtd, unitTotalPrice=$unitTotalPrice, percDiscountCommission=$percDiscountCommission, valueDiscount=$valueDiscount, percDiscount=$percDiscount, useValueDiscount=$useValueDiscount, discountType=$discountType, discountByWeight=$discountByWeight, discountTotal=$discountTotal]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "PurchaseOrderItem[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "PurchaseOrderItem[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PurchaseOrderItem> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<PurchaseOrderItem>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PurchaseOrderItem.listFromJson(
          entry.value,
          growable: growable,
        );
      }
