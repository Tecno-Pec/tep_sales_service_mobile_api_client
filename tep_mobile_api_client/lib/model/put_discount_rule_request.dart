...
class PutDiscountRuleRequest {
  /// Returns a new [PutDiscountRuleRequest] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PutDiscountRuleRequest &&
  ...
  @override
  String toString() => 'PutDiscountRuleRequest[createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, id=$id, discountWeightType=$discountWeightType, minQuantity=$minQuantity, discountType=$discountType, referenceType=$referenceType, referenceId=$referenceId, discount=$discount, externalCode=$externalCode, status=$status]';
  ...
  /// Returns a new [PutDiscountRuleRequest] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PutDiscountRuleRequest? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PutDiscountRuleRequest[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PutDiscountRuleRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return PutDiscountRuleRequest(
  ...
  static List<PutDiscountRuleRequest> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PutDiscountRuleRequest>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PutDiscountRuleRequest.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PutDiscountRuleRequest> mapFromJson(dynamic json) {
    final map = <String, PutDiscountRuleRequest>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PutDiscountRuleRequest.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PutDiscountRuleRequest-objects as value to a dart map
  static Map<String, List<PutDiscountRuleRequest>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PutDiscountRuleRequest>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PutDiscountRuleRequest.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
