...
class PostUserResponse {
  /// Returns a new [PostUserResponse] instance.
  PostUserResponse({
    this.createdAt,
    this.updatedAt,
    this.userCreated,
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is PostUserResponse &&
  ...
  @override
  String toString() => 'PostUserResponse[createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, id=$id, name=$name, documentId=$documentId, phone=$phone, email=$email, isActive=$isActive, isAdmin=$isAdmin, isSuperAdmin=$isSuperAdmin, isSalesman=$isSalesman, isIntegration=$isIntegration, isRegionManager=$isRegionManager, externalCode=$externalCode, status=$status]';
  ...
  /// Returns a new [PostUserResponse] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static PostUserResponse? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "PostUserResponse[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "PostUserResponse[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PostUserResponse> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <PostUserResponse>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = PostUserResponse.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, PostUserResponse> mapFromJson(dynamic json) {
    final map = <String, PostUserResponse>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = PostUserResponse.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of PostUserResponse-objects as value to a dart map
  static Map<String, List<PostUserResponse>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<PostUserResponse>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = PostUserResponse.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }
