...
class PostClientRequest {
  ...
  @override
  String toString() =>
      'PostClientRequest[createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, id=$id, name=$name, document=$document, phone=$phone, ie=$ie, email=$email, externalCode=$externalCode, status=$status, userSellerId=$userSellerId, paymentConditionId=$paymentConditionId]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "PostClientRequest[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "PostClientRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PostClientRequest> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<PostClientRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PostClientRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
