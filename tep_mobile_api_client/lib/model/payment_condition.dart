...
class PaymentCondition {
  ...
  @override
  String toString() =>
      'PaymentCondition[id=$id, createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, version=$version, companyId=$companyId, name=$name, externalCode=$externalCode, sequence=$sequence, status=$status]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "PaymentCondition[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "PaymentCondition[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PaymentCondition> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<PaymentCondition>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PaymentCondition.listFromJson(
          entry.value,
          growable: growable,
        );
      }
