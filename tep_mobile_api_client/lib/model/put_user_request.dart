...
class PutUserRequest {
  ...
  @override
  String toString() =>
      'PutUserRequest[createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, id=$id, name=$name, documentId=$documentId, phone=$phone, email=$email, isActive=$isActive, isAdmin=$isAdmin, isSuperAdmin=$isSuperAdmin, isSalesman=$isSalesman, isIntegration=$isIntegration, isRegionManager=$isRegionManager, externalCode=$externalCode, status=$status]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "PutUserRequest[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "PutUserRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PutUserRequest> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<PutUserRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PutUserRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
