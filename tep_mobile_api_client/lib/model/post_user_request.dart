...
class PostUserRequest {
  ...
  @override
  String toString() =>
      'PostUserRequest[createdAt=$createdAt, updatedAt=$updatedAt, userCreated=$userCreated, userUpdated=$userUpdated, id=$id, name=$name, documentId=$documentId, phone=$phone, email=$email, isActive=$isActive, isAdmin=$isAdmin, isSuperAdmin=$isSuperAdmin, isSalesman=$isSalesman, isIntegration=$isIntegration, isRegionManager=$isRegionManager, externalCode=$externalCode, status=$status, password=$password]';
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key),
              'Required key "PostUserRequest[$key]" is missing from JSON.');
          assert(json[key] != null,
              'Required key "PostUserRequest[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<PostUserRequest> listFromJson(
    dynamic json, {
    bool growable = false,
  }) {
  ...
  static Map<String, List<PostUserRequest>> mapListFromJson(
    dynamic json, {
    bool growable = false,
  }) {
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = PostUserRequest.listFromJson(
          entry.value,
          growable: growable,
        );
      }
