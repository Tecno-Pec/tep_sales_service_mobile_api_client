import 'package:test/test.dart';
import 'package:tep_mobile_api_client/tep_mobile_api_client.dart';

// tests for PutPurchaseOrderRequest
void main() {
  final instance = PutPurchaseOrderRequestBuilder();
  // TODO add properties to the builder and call build()

  group(PutPurchaseOrderRequest, () {
    // DateTime createdAt
    test('to test the property `createdAt`', () async {
      // TODO
    });

    // DateTime updatedAt
    test('to test the property `updatedAt`', () async {
      // TODO
    });

    // String userCreated
    test('to test the property `userCreated`', () async {
      // TODO
    });

    // String userUpdated
    test('to test the property `userUpdated`', () async {
      // TODO
    });

    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // double freightValue
    test('to test the property `freightValue`', () async {
      // TODO
    });

    // double unloadingValue
    test('to test the property `unloadingValue`', () async {
      // TODO
    });

    // FreightType freightType
    test('to test the property `freightType`', () async {
      // TODO
    });

    // double valueWithoutFreight
    test('to test the property `valueWithoutFreight`', () async {
      // TODO
    });

    // double totalValue
    test('to test the property `totalValue`', () async {
      // TODO
    });

    // double discount
    test('to test the property `discount`', () async {
      // TODO
    });

    // double distance
    test('to test the property `distance`', () async {
      // TODO
    });

    // double earthDistance
    test('to test the property `earthDistance`', () async {
      // TODO
    });

    // int totalWeightkilograms
    test('to test the property `totalWeightkilograms`', () async {
      // TODO
    });

    // DateTime dueDate
    test('to test the property `dueDate`', () async {
      // TODO
    });

    // DateTime mobileCreatedAt
    test('to test the property `mobileCreatedAt`', () async {
      // TODO
    });

    // bool isFreightFractional
    test('to test the property `isFreightFractional`', () async {
      // TODO
    });

    // String integrationCode
    test('to test the property `integrationCode`', () async {
      // TODO
    });

    // String paymentConditionId
    test('to test the property `paymentConditionId`', () async {
      // TODO
    });

    // String clientId
    test('to test the property `clientId`', () async {
      // TODO
    });

    // String deliveryAddressId
    test('to test the property `deliveryAddressId`', () async {
      // TODO
    });

    // String invoiceAddressId
    test('to test the property `invoiceAddressId`', () async {
      // TODO
    });

    // BuiltList<PurchaseOrderItemModelBase> items
    test('to test the property `items`', () async {
      // TODO
    });

    // BuiltList<FreightCompositionModel> freightCompositions
    test('to test the property `freightCompositions`', () async {
      // TODO
    });

    // String mobileCreatedUserId
    test('to test the property `mobileCreatedUserId`', () async {
      // TODO
    });

    // String paymentPriceTableId
    test('to test the property `paymentPriceTableId`', () async {
      // TODO
    });

    // String distribuitionCenterId
    test('to test the property `distribuitionCenterId`', () async {
      // TODO
    });

    // String distribuitionCenterClientAddressId
    test('to test the property `distribuitionCenterClientAddressId`', () async {
      // TODO
    });

  });
}
